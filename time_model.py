...
def calc_meeting_time(total_meeting_hours, as_prop: bool = False, rounding: int = ROUNDING_DEFAULT, total_hours_in_week: int = TOTAL_WORKING_HOURS_IN_WEEK):
    val = total_meeting_hours
    if as_prop:
        val = val / total_hours_in_week
    return round(float(val), rounding)

def calc_lost_productivity(total_meeting_blocks, context_switch_cost_mins, as_prop: bool = False, rounding: int = ROUNDING_DEFAULT, total_hours_in_week: int = TOTAL_WORKING_HOURS_IN_WEEK):
    val = round(float(total_meeting_blocks * context_switch_cost_mins / 60), rounding)
    if as_prop:
        val = val / total_hours_in_week
    return round(float(val), rounding)

def calc_productive_time_lost(total_meeting_hours, total_meeting_blocks, context_switch_cost_mins, as_prop: bool = False, rounding: int = ROUNDING_DEFAULT, total_hours_in_week: int = TOTAL_WORKING_HOURS_IN_WEEK):
    val = total_meeting_hours + calc_lost_productivity(total_meeting_blocks, context_switch_cost_mins)
    if as_prop:
        val = val / total_hours_in_week
    return round(float(val), rounding)

def calc_potential_productive_time(total_meeting_hours, total_meeting_blocks, context_switch_cost_mins, as_prop: bool = False, rounding: int = ROUNDING_DEFAULT, total_hours_in_week: int = TOTAL_WORKING_HOURS_IN_WEEK):
